...
function computeNewSelection(textarea: HTMLTextAreaElement, action: ToggleAction, markerLength: number, position: DecorationPosition, keepCaretPos: boolean) {
          ...
          const transformed = lines
            .map((line) => {
              if (line.startsWith(tabSpace))
                return line.slice(tabSpace.length)
              if (line.startsWith('\t'))
                return line.slice(1)
              return line
            })
  ...
  const handleListContinuation = useCallback(
    (event: React.KeyboardEvent<HTMLTextAreaElement>, target: HTMLTextAreaElement) => {
      ...
      if (event.key === 'Enter' && !event.shiftKey && !event.metaKey && !event.ctrlKey) {
        ...
        if (handleEmptyListItem(event, currentLine))
          return
