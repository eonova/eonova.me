...
const Books: React.FC = () => {
  const { data, status } = api.books.getBookData.useQuery({
    userId: '271041273',
      ...
      />
      {
        ...
        <div>
          <div className="mb-8 flex space-x-4">
            ...
            >
              想看 ({data?.data?.user?.stats?.book?.wish})
            </button>
            <button
              onClick={() => {
                setCurrentTab('do');
              ...
              className={`px-6 py-2 rounded-lg ${currentTab === 'do'
                ? 'bg-blue-500 text-white'
                : 'bg-gray-200 hover:bg-gray-300'
                }`}
            >
              正在看 ({data?.data?.user?.stats?.book?.do})
            </button>
            <button
              onClick={() => {
            ...
            >
              看过 ({data?.data?.user?.stats?.book?.collect})
            </button>
          </div>
          {/* 电影网格布局 */}
