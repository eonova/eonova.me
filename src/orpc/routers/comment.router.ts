...
import { and, asc, comments, count, desc, eq, gt, isNotNull, isNull, lt, ne, unsubscribes, votes } from '~/db'
...
export const createComment = protectedProcedure
  ...
  .handler(async ({ input, context }) => {
    ...
    const { contentType, slug, content, parentId, date } = input
    ...
    const comment = await context.db.transaction(async (tx) => {
      ...
      if (IS_PRODUCTION && input.parentId) {
        ...
        if (parentComment && parentComment.userId !== user.id && parentComment.user.id !== 'ghost') {
          ...
          if (unsubscribedFromAllReplies || unsubscribedFromThisComment)
            return c
...
export const countComments = publicProcedure
  ...
  .handler(async ({ input, context }) => {
    const [result] = await context.db
      ...
      .from(comments)
      .where(and(eq(comments.contentId, input.slug), input.withReplies ? undefined : isNull(comments.parentId)))
