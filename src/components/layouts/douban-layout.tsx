...
function DoubanLayout(props: Readonly<DoubanLayoutProps>) {
  const { MODES, data, status, isRefetching, pageSize = 16, isFlat = false } = props
  ...
  // 获取当前选项卡数据
  const currentCollection = (data as DoubanDataResponse)?.data?.collections?.find(
    c => c.action === selectedMode,
  )
        ...
        {MODES.map(mode => (
          ...
          >
            ...
            {!isFlat
              ...
              : (getFlatArrLength(
                  data?.data?.collections.find(c => c.action === mode)?.items ?? [],
                ) ?? 0)}
          </button>
        ))}
      </div>
